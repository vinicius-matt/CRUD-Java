          ...
          if (dadosClientes.length > 0) {
            ...
            xhrDelete.onreadystatechange = function () {
              if (xhrDelete.readyState == 4) {
                ...
                console.log("ID " + idsObtidos.id + " excluído com sucesso. Status:", xhrDelete.status);
                } else if (xhrDelete.status == 404) {
                console.error("Cliente com ID " + idsObtidos.id + " não encontrado. Status:", xhrDelete.status);
                } else {
                console.error("Erro ao excluir ID " + idsObtidos.id + ". Status:", xhrDelete.status);
